...
public class Solution55 {
    ...
    /*
    法二：法一的回溯使用递归超时，那可能就要用循环。这又是一个数组，那就是遍历呗！所以思考遍历的时候应该干啥？
    记录并更新什么变量？注意到，对于每一个可达的位置i，按照题意，其后面的i+1,i+2,...,i+num[i]这些位置都
    可以到达。因此，我们可以遍历数组，并实时更新遍历到该位置时最远可以到达的位置！对于当前遍历到的位置x，
    如果它在当前最远可达位置范围内，说明可以从起点通过若干次跳跃跳到该位置，并可以继续往前跳，
    并且如果x+nums[x]大于当前最远可达位置，应该更新最远可达位置。
    时间复杂度为O(N)!空间复杂度为O(1)
     */
